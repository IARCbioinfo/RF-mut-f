...
vcf_stats=$(patsubst %.vcf.gz,%.stats,$(wildcard *.vcf.gz))
%.stats:%.vcf.gz
	${TABIX} $<
	${BCFTOOLS} stats -r ${CHRS}  $< > $@
stats:$(vcf_stats)
#compute shared SNPs/INDELS with vars
shared_stats=$(patsubst %.vcf.gz,%.shared.stats,$(wildcard *.vcf.gz))
%.shared.stats:%.vcf.gz
	 ${BCFTOOLS} stats -r ${CHRS} $< ../matched/$(subst .tonly.vcf.gz,.vcf.gz,$<) > $@
shared : $(shared_stats)
...
COSMIC_DB=/data/scratch/digenovaa/Somatic-reference-free/cosmic/CosmicSNV_INDELS.gnomad.vcf.bgz
...
%.centro_cgenes.vcf.bgz:%.cosmic.vcf.bgz
	${BCFTOOLS} annotate -a centromers.bed.gz -h centromers.hdr -c CNAME --columns CHROM,FROM,TO,CNAME -m +CENTROMER $< | \
	${BCFTOOLS} annotate -a cosmic_gene_census.bed.gz -h cosmic_gene_census.hdr -c CGENE --columns CHROM,FROM,TO,CGENE -O z -o $@ -m +COSMIC_CENSUS_GENE -
	tabix $@
centro_genes:$(add_centro_cgenes)

#annotate variants effects
RFASTA=/data/scratch/digenovaa/Somatic-reference-free/SNV-INDELs/programs/ensembl/Homo_sapiens.GRCh38.dna.primary_assembly.fa
RGFF3=/data/scratch/digenovaa/Somatic-reference-free/SNV-INDELs/programs/ensembl/Homo_sapiens.GRCh38.99.chr.gff3.gz

add_effect=$(patsubst %.centro_cgenes.vcf.bgz,%.csq.vcf.bgz,$(add_centro_cgenes))
%.csq.vcf.bgz: %.centro_cgenes.vcf.bgz
	${BCFTOOLS} annotate --rename-chrs chr-names2.txt $< | ${BCFTOOLS} csq -p a -O z -o $@ -f ${RFASTA} -g ${RGFF3} -
add_csq:$(add_effect)

#we mark the SOMATICS Vars and annot commo vars
...
CMF=/home/digenovaa/scratch/Somatic-reference-free/SNV-INDELs/code/iarcbioinfo/TON-Calling/VCF/extract_feature_from_VCF.pl
...
all: stats shared gnomad_annot comic_annot centro_genes add_csq som split to_matrix Somatics.snv.matrix.txt Somatics.snv.matrix.WMESO_061.txt
