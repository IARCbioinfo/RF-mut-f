...
#default is 0.5-0.5, 0.7 might be more appropiate norm.votes=TRUE
predicted.class <- predict(RF, sample.features, cutoff = c(arguments$cutoff, 1-arguments$cutoff));
#we add prob
predicted.prob <- predict(RF, sample.features, type="prob",  cutoff = c(arguments$cutoff, 1-arguments$cutoff));
#we add number of votes for each prediction
predicted.votes <- predict(RF, sample.features, type="vote", norm.votes=FALSE, cutoff = c(arguments$cutoff, 1-arguments$cutoff));
